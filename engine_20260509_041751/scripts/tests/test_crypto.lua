...
local function run()
    ...
    T.test("crypto.xxhash64: 对齐Go cespare/xxhash/v2 测试向量", function()
        -- 来自 github.com/cespare/xxhash/v2 官方测试
        local vectors = {
            { "",     "ef46db3751d8e999" },
            { "a",    "d24ec4f1a98c6e5b" },
            { "as",   "1c330fb2d66be179" },
        ...
        for _, v in ipairs(vectors) do
            local got = crypto.xxhash64(v[1])
            T.assert_eq(got, v[2], "xxhash64('" .. v[1]:sub(1,20) .. "...') mismatch")
        end
        T.log("  所有 6 个 Go xxhash 测试向量通过")
    end)
