...
-- 测试配置
--============================================================================
local TEST_LICENSE = "LUA5D744CAA2A7A601FB14514E3D93B9D5A"
...
function M.run()
    ...
    if not driverLoaded then
        T.test(
            "driver.load: 使用卡密加载驱动",
            function()
                local hwid = sys.hwid()
                T.trace("sys.hwid", {}, hwid)
                local ok, err = driver.load(TEST_LICENSE)
                T.trace("driver.load", {TEST_LICENSE}, ok, err)
                if ok then
                    driverLoaded = true
                end
            end
