...
>
> | | `proc.call` | `driver.init_call` + `driver.exec_call` |
> |---|---|---|
> | **执行方式** | 用户态 APC（NtQueueApcThreadEx2） | 驱动级远程调用 |
> | **执行上下文** | 目标进程主线程 | 驱动分配的线程 |
...
| 函数 | 返回 | 说明 |
|------|------|------|
| `driver.init_call(pid, hwnd?)` | shared_buf | 初始化远程调用，返回目标进程共享缓冲区地址 |
| `driver.exec_call(pid, func, ...)` | ret, err | 执行远程调用 (最多16个u64参数) |

**远程调用说明：**

1. `init_call` 在目标进程分配共享内存，返回缓冲区基址（可通过 `driver.write_memory` 写入参数数据）
2. `exec_call` 在目标进程上下文执行指定函数地址，可变参数为 u64 整数（指针/整数均可）
3. 字符串参数需先 `write_memory` 写入共享缓冲区，再把缓冲区地址作为参数传入

```lua
-- 远程调用示例: 调用目标进程的 MessageBoxA
