...
`driver.load()` 不需要卡密；旧脚本传入的卡密参数会被忽略。
...
如果脚本作者不擅长设计易用界面，建议直接以 `scripts/test_ui_multiplay.lua` 作为二次开发模板。这个 demo 是一个单机多开挂机控制面板，已经包含实际项目常用的 UI 结构：

- 账号列表表格：展示账号、密码、区服、角色名、等级、金币、状态、脚本等字段。
- 批量选择：支持全选、单选、Shift 连续多选。
- 脚本分配：通过下拉框选择脚本，并批量应用到选中的账号。
- 控制按钮：开始、停止、账号导入、账号导出、添加、删除、编辑、保存。
- 状态展示：使用不同颜色显示运行、暂停、卡点、未启动等状态。
- 操作日志：保留最近操作记录，方便用户理解脚本当前行为。
- 数据导入导出：示例使用 `accounts.txt` 保存账号表，便于替换成自己的配置格式。

二次开发时推荐保留它的整体布局：顶部批量操作，中间账号表格，底部日志。开发者通常只需要替换账号字段、脚本列表和开始/停止逻辑，就能得到一个对用户更友好的控制面板。

```lua
-- 运行示例
-- 编辑器模式: AetherRunner.exe --gui 后运行 scripts/test_ui_multiplay.lua
-- 命令行模式: AetherRunner.exe scripts/test_ui_multiplay.lua
```

发布给用户时，可以把它改名为自己的主界面脚本，并在 `config.json` 中设置：

```json
{
  "mainScript": "scripts/test_ui_multiplay.lua",
  "enableWindow": true
}
```
