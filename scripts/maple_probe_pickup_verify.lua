...
return Probe.pickup_verify({
    ...
    action_wait_ms = tonumber(probe_action_wait_ms) or 120,
    verify_waits = probe_verify_waits or "0,200,500,1000,2000",
    max_drop_log = tonumber(probe_max_drop_log) or 10,
    max_key_log = tonumber(probe_max_key_log) or 6,
    pickup_api_enabled = probe_pickup_api_enabled ~= false,
    pickup_key_enabled = probe_pickup_key_enabled == true,
