...
local SCRIPT_FILE = script_file_path() or "AvePointStandalone_v2.lua"
...
local function main()
    log.info("AvePoint standalone v2 entry starting")
    extend_package_path(ENTRY_SCRIPT)

    local chunk, err = load_script_chunk(ENTRY_SCRIPT)
    if not chunk then
        error("AvePoint standalone v2 load failed: " .. tostring(err or ENTRY_SCRIPT))
    end
