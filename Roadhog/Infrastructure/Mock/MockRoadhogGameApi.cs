...
internal sealed class MockRoadhogGameApi : IRoadhogGameApi
{
    ...
    {
        return Task.FromResult(OperationResult<SummonedPetSnapshot>.Ok(
            SummonedPetSnapshot.NotSummoned(0, DateTimeOffset.Now)));
    }

    public Task<OperationResult<SummonedPetRosterSnapshot>> ReadSummonedPetRosterAsync(CancellationToken cancellationToken = default)
    {
        return Task.FromResult(OperationResult<SummonedPetRosterSnapshot>.Ok(
    ...
    public Task<OperationResult<LockedTargetAbnormalStatusSnapshot>> ReadLockedTargetAbnormalStatusesAsync(
        CancellationToken cancellationToken = default)
    {
        return Task.FromResult(OperationResult<LockedTargetAbnormalStatusSnapshot>.Ok(
            new LockedTargetAbnormalStatusSnapshot(
    ...
    {
        return Task.FromResult(OperationResult<GatherSnapshot>.Ok(GatherSnapshot.Empty(DateTimeOffset.Now)));
    }
