...
public static class CleanupTradePolicy
{
    ...
    public static bool ReservedForAuction(InventoryItemSnapshot item, MaintenanceScriptSettings settings) =>
        settings.BagCleanupAuctionHouseItems.Any(r => Matches(item.Name, r.Name));
    public static bool Reserved(InventoryItemSnapshot item, MaintenanceScriptSettings settings) =>
        ReservedForAuction(item, settings) || settings.BagCleanupStallItems.Any(r => Matches(item.Name, r.Name));
