...
角色空闲
  ...
  -> 节点附近安全清怪
  -> 超过 20 米则寻路，进入 20 米后按键
  -> 按该类别自己的采集键
  -> 从第一次成功按键开始，最多等待 5 秒出现 DlgGathering
  -> 当前节点激活或 DlgGathering 可见期间，不执行红蓝、状态或坐下维护
  -> 节点仍存在则继续下一次
...
- 如果安全半径内的主动怪正在攻击别人，或已进入脚本忽略名单，则不再阻止采集。
- 已忽略怪一旦改为攻击本机或本机召唤物，仍立即交回战斗链，不会继续采集。
- 采集键已经发出或读条可见后，普通附近主动怪不再打断；只有明确攻击本机或本机召唤物的活怪会切回战斗。
- 被动怪和安全半径外、未攻击本机的主动怪不阻止本次采集。

失败边界：

- 怪物、竞争或本机读条数据任一不可用时，不按采集键。
- 已锁定节点后，第一次真实采集快照不可用时保留节点并阻止回落打怪；连续两次不可用才短时抑制该节点，任意一次成功读取会清零。
- 第一次成功按键后最多等待 5 秒确认读条；输入连续三次失败时短时抑制该具体节点，防止死循环。
- 接近阶段会检测无位移、尝试跳跃；持续无法接近则抑制节点。
- 采集物单次读取缺失不算采空；相同 `CapturedAt` 的缓存快照不会重复累计。
- 已按键等待读条期间，采集物短暂缺失不累计采空次数；读条出现或等待超时后再按既有流程处理。
